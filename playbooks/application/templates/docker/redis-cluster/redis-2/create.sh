...
docker run --init -d \
           ...
           -v "{{docker_files_path}}/redis-cluster/redis-2/data:/data" \
           -v "{{docker_files_path}}/redis-cluster/redis-2/config/redis.conf:/opt/bitnami/redis/mounted-etc/redis.conf:ro" \
